...
function calculateIntegrity(content: string | Buffer): string {
  try {
    ...
    throw new ModuleIntegrityError(`Failed to calculate integrity: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function generateImportMapWithIntegrity(
  ...
  invariant(Object.keys(modules).length > 0, 'No modules provided for integrity calculation');
...
function readModulesFromDirectories(
  ...
  invariant(directories.length > 0, 'No directories specified for module scanning');
  invariant(extensions.length > 0, 'No extensions specified for module scanning');
    ...
    for (const file of files) {
      if (extensions.includes(path.extname(file))) {
        try {
          ...
          throw new ModuleIntegrityError(`Failed to read file ${file}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
  }
...
function writeImportMap(importMap: ImportMap, outputPath: string): void {
  ...
  try {
    ...
    throw new ModuleIntegrityError(`Failed to write import map to ${outputPath}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export class ModuleIntegrityPlugin implements webpack.WebpackPluginInstance {
  ...
  constructor(config: IntegrityConfig) {
    invariant(config.extensions?.length > 0, 'Module extensions must be specified');
  ...
  apply(compiler: webpack.Compiler): void {
    ...
    compiler.hooks.compilation.tap('ModuleIntegrityPlugin', (compilation) => {
      ...
      compilation.hooks.afterOptimizeChunkModules.tap('ModuleIntegrityPlugin', (chunks) => {
        try {
          ...
          chunks.forEach(chunk => {
            ...
            const chunkModules = Array.from(chunk.modulesIterable || chunk.getModules());
            
            chunkModules.forEach(module => {
              this.processModule(module, moduleAssets, processedModules, compilation);
            });
          });
          ...
          console.log(`[ModuleIntegrityPlugin] Generated import map with ${Object.keys(importMap.integrity || {}).length} integrity hashes from ${processedModules.size} webpack modules`);
        } catch (error) {
          console.error(error instanceof Error ? error.message : String(error));
          if (error instanceof ModuleIntegrityError) {
            // We don't want to fail the build for integrity errors
            console.error('[ModuleIntegrityPlugin] Failed to generate import map');
          } else {
    ...
    if (this.shouldIncludeModule(module.resource)) {
      try {
        ...
        if (typeof module.originalSource === 'function') {
          ...
          source = typeof module.source === 'function' 
            ? module.source().source()
            : module.source;
        } else if (compilation.moduleGraph) {
          // For webpack 5
          const moduleSource = compilation.codeGenerationResults?.get(module)?.sources?.get('javascript');
          if (moduleSource) {
        ...
        this.processDependencies(module, moduleAssets, processedModules, compilation);
      } catch (error) {
        console.warn(`[ModuleIntegrityPlugin] Failed to process module ${module.resource}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
  ...
  private processDependencies(
    ...
    if (compilation.moduleGraph) {
      // Webpack 5
      dependencies = Array.from(compilation.moduleGraph.getOutgoingConnections(module) || [])
    ...
    dependencies.forEach(depModule => {
      this.processModule(depModule, moduleAssets, processedModules, compilation);
    });
  }
