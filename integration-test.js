...
describe("Module Integrity", () => {
    ...
    `;
		...
		global.crypto = {
			subtle: {
				digest: jest.fn((algorithm, data) => {
					...
					const sum = Array.from(dataArray).reduce(
						(sum, byte) => sum + byte,
						0,
					);
		...
		mockResponses["../resources/log.js?pipe=sub&name=B"] =
			'console.log("Module B loaded")';
		mockResponses["../resources/log.js?pipe=sub&name=D"] =
			'console.log("Module D loaded")';
		mockResponses["../resources/log.js?pipe=sub&name=E"] =
			'console.log("Module E loaded")';
		mockResponses["../resources/log.js?pipe=sub&name=F"] =
	...
	beforeEach(() => {
		...
		importWithIntegrity = async (specifier) => {
			...
			if (!integrity) {
				...
				for (const [bare, targetUrl] of Object.entries(importMap.imports)) {
					if (targetUrl === url && importMap.integrity[bare]) {
						throw new Error(
							`Integrity check failed for module: ${url} (via ${bare})`,
						);
					}
				}
			...
			const actualIntegrity = await calculateIntegrity(
				content,
				integrity.split("-")[0],
			);
			...
			if (actualIntegrity !== integrity) {
				...
				return Promise.reject(
					new Error(`Integrity check failed for module: ${url}`),
				);
			}
	...
	test("script was not loaded, as it failed its integrity check", async () => {
		await expect(
			importWithIntegrity("../resources/log.js?pipe=sub&name=A"),
		).rejects.toThrow();
	...
	test("Bare specifier script was not loaded, as it failed its integrity check", async () => {
		global.log = [];
		await expect(importWithIntegrity("bare")).rejects.toThrow();
		expect(global.log.length).toBe(0);
	});
	...
	test("Even though G does not have an integrity check, the resolved URL does through its bare specifier", async () => {
		global.log = [];
		await expect(
			importWithIntegrity("../resources/log.js?pipe=sub&name=G"),
		).rejects.toThrow();
		expect(global.log.length).toBe(0);
	});
});
